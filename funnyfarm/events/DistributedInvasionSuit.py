...
class DistributedInvasionSuit(DistributedSuitBase, InvasionSuitBase, FSM, DelayDeletable):
    ...
    def takeDamage(self, hp):
        ...
        self.enterStunned()
        damageSeq = Sequence(
            Wait(self.stunInterval.getDuration()), 
            Func(self.exitStunned), 
            Func(self.enterExplode)
        )
        damageSeq.start()

    def enterStunned(self):
        self.stunInterval = ActorInterval(self, 'pie-small-react')
        self.stunInterval.start()

    def exitStunned(self):
        self.stunInterval.finish()
    ...
    def enterExplode(self):
        ...
        # Blow the sucker up
        self.explodeTrack = Sequence(Parallel(explosionInterval, deathSoundTrack, gears1Track, gears2MTrack), Func(self.cleanupLoseActor))
