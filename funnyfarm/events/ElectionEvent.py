...
from DistributedInvasionSuit import DistributedInvasionSuit
...
'''
Since this is single player, and we're not using a ClientRepository or AI system, 
I've taken TTR's DElectionEvent and DElectionEventAI, combined the two, and removed
all references to an FSM, CR, or DObject. The same goes for the other classes in this folder.
'''

class ElectionEvent(DirectObject):
	...
	def __init__(self):
		...
		self.showFloor.reparentTo(base.render)
		...
		# Since Disney didn't include Prepostera's animation for Surlee's size, we have to make him small and painstakingly scale him
		...
		self.suit = DistributedInvasionSuit(None)
		suitDNA = SuitDNA.SuitDNA()
		suitDNA.newSuit('cr')
		self.suit.setDNA(suitDNA)
		self.suit.setDisplayName('Corporate Raider\nBossbot\nLevel 11')
	...
	def __cleanupNPCs(self):
		npcs = [self.flippy, self.slappy, self.alec, self.surlee, self.surleeR, self.prepostera, self.dimm]
		for npc in npcs:
			if npc:
				npc.removeActive()
	...
	def stopInteractiveFlippy(self):
		self.interactiveOn = False
		self.flippy.reparentTo(hidden)

	'''
	 ELECTION DAY SEQUENCE
	...
	def loadSequences(self):
		...
		self.surleeLeaveInterval = Sequence(
			...
			Wait(1),
			Parallel(Func(self.surleeR.loop, 'walk'), self.surleeR.posHprInterval(4, (-20, 0, 0), (190, 0, 0))),
		...
		self.cogSequence = Sequence(
			Parallel(Func(self.suit.reparentTo, render), Func(self.suit.addActive), Func(mtrack.start)),
			...
			Func(self.exitElectionIdle), 
			Func(self.enterElectionCrashing), 
			Wait(2),
			...
			Wait(0.5),
			Func(self.slappyStandDie.start),
		...
		self.surleeIntroInterval = Sequence(
			...
			Parallel(
				...
				Func(self.surleeAnimation.loop)
			),
	...
	def startElectionEvent(self, offset):
		if self.surleeIntroInterval:
			self.exitIdle()
		self.loadSequences()
		preShowDur = self.surleeLeaveInterval.getDuration()
		beginDur = self.flippyHallInterval.getDuration()
		speechDur = self.alecSpeech.getDuration()
		voteDur = self.buildupSequence.getDuration()
		victoryDur = self.victorySequence.getDuration()
		self.electionSequence = Sequence(
			Func(self.enterPreShow), 
			Wait(preShowDur),  
			Func(self.enterBegin), 
			Wait(beginDur), 
			Func(self.exitBegin), 
			Func(self.enterAlecSpeech), 
			Wait(speechDur), 
			Func(self.exitAlecSpeech), 
			Func(self.enterVoteBuildup), 
			Wait(voteDur), 
			Func(self.enterWinnerAnnounce), 
			Wait(victoryDur), 
			Func(self.exitWinnerAnnounce), 
			Func(self.enterCogLanding), 
			Wait(117), 
			Func(self.exitCogLanding), 
			Func(self.enterInvasion)
		)
		self.electionSequence.start()
		self.electionSequence.setT(offset)
	...
	def setSuitDamage(self, hp):
		if not self.cogDead:
			...
			Sequence(Wait(2.25), Func(self.suit.hide), Func(self.suit.removeActive)).start()
